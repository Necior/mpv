...
#define GL_VDA_MAX_PLANES 3

struct vda_format {
    uint32_t cvpixfmt;
    int imgfmt;
    int planes;
    struct vda_gl_plane_format gl[GL_VDA_MAX_PLANES];
};

struct priv {
    CVPixelBufferRef pbuf;
    GLuint gl_planes[GL_VDA_MAX_PLANES];
...
#endif
    struct mp_vda_ctx *vda_ctx = talloc_zero(p, struct mp_vda_ctx);
    *vda_ctx = (struct mp_vda_ctx) { .cvpixfmt = f->cvpixfmt, };
    hw->converted_imgfmt = f->imgfmt;
    p->hwctx.vda_ctx = vda_ctx;
    ...
    GL *gl = hw->gl;
    gl->GenTextures(GL_VDA_MAX_PLANES, p->gl_planes);
...
static void destroy(struct gl_hwdec *hw)
{
    ...
    CVPixelBufferRelease(p->pbuf);
    gl->DeleteTextures(GL_VDA_MAX_PLANES, p->gl_planes);
}
